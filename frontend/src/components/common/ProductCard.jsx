...
const ProductCard = ({ product, index }) => {
    ...
    return (
        <motion.div
            ...
            className="group relative bg-secondary/30 rounded-[2.5rem] overflow-hidden transition-colors hover:bg-secondary/50 shadow-sm border border-secondary"
        >
            {/* Wishlist Button */}
            <button
                onClick={(e) => {
                ...
                className="absolute top-6 right-6 z-20 w-10 h-10 glass rounded-full flex items-center justify-center transition-colors group/heart shadow-sm"
            >
                <Heart
                    size={18}
                    ...
                    <AnimatePresence>
                        {isHovered && (
                            <motion.div
                                ...
                                animate={{ opacity: 1 }}
                                exit={{ opacity: 0 }}
                                className="absolute inset-0 bg-black/5 backdrop-blur-[2px] flex items-center justify-center gap-4"
                            >
                                <motion.button
                                    initial={{ y: 20, opacity: 0 }}
                                    animate={{ y: 0, opacity: 1 }}
                                    whileHover={{ scale: 1.1 }}
                                    className="w-12 h-12 bg-white text-black rounded-full flex items-center justify-center shadow-lg hover:text-accent transition-colors"
                                >
                                    <Eye size={20} />
                                </motion.button>
                            </motion.div>
